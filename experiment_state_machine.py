...
# Ta program skrbi za sledenje in določanje stanj v katerih se nahaja trial. vsi možni stati so definirani v StateMachinee classu ki določa naslednje stanje in naslednji trial glede na kvazi slučajen seznam eventov.
...
class StateMachinee:
    ...
    def __init__(self, trial_No = 20, time_delay = [1, 3]):
        ...
        all_variables = vars(StateMachinee)
    ...
    def maybe_update_state(self, state_dict):
        ...
        if self.current_state is None:
            self.current_state = StateMachinee.WAITING_FOR_START
            self.set_waiting_for_start(state_dict)

        #### AT THE BEGINNING
        elif self.current_state == StateMachinee.WAITING_FOR_START:
            if state_dict["eduexo_online"] == True:
                self.current_state = StateMachinee.INITIAL_SCREEN
                self.set_initial_screen(state_dict)

        elif self.current_state == StateMachinee.INITIAL_SCREEN:
            if state_dict["enter_pressed"]:
                self.current_state = StateMachinee.GO_TO_MIDDLE_CIRCLE
                self.set_start_experiment(state_dict)
                self.set_go_to_middle_circle(state_dict)

        elif self.current_state == StateMachinee.GO_TO_MIDDLE_CIRCLE:
            if np.linalg.norm(state_dict["main_circle_position"] - state_dict["middle_circle_position"]) < state_dict["middle_circle_radius"]: # spremeni v "loc" in kalibriraj na zacetni krog
                self.current_state = StateMachinee.IN_MIDDLE_CIRCLE
        ...
        elif self.current_state == StateMachinee.IN_MIDDLE_CIRCLE:
            if time() - state_dict["state_start_time"] >= state_dict["state_wait_time"]:
                self.current_state = StateMachinee.GO_TO_UPPER_BAND_AFTER_TRIAL
        ...
        elif self.current_state == StateMachinee.IN_UPPER_BAND:
            if time() - state_dict["state_start_time"] >= state_dict["state_wait_time"]:
                self.current_state = StateMachinee.GO_OUT_OF_UPPER_BAND
        ...
        elif self.current_state == StateMachinee.IN_LOWER_BAND:
            if time() - state_dict["state_start_time"] >= state_dict["state_wait_time"]:
                self.current_state = StateMachinee.GO_OUT_OF_LOWER_BAND
                ...
                self.current_state = StateMachinee.GO_TO_LOWER_BAND_AFTER_TRIAL
        ...
        #### WHEN TRIAL STARTS
        elif self.current_state == StateMachinee.GO_OUT_OF_UPPER_BAND:
            state_dict["remaining_time"] -= time() - self.prev_time
            self.prev_time = time()

            if np.linalg.norm(state_dict["main_circle_position"] - state_dict["upper_band_position"]) >= state_dict["upper_band_radius"]:
                self.current_state = StateMachinee.GO_TO_LOWER_BAND
                self.set_go_to_lower_band(state_dict)
                self.prev_time = time()

        elif self.current_state == StateMachinee.GO_OUT_OF_LOWER_BAND:
            state_dict["remaining_time"] -= time() - self.prev_time
            self.prev_time = time()

            if np.linalg.norm(state_dict["main_circle_position"] - state_dict["lower_band_position"]) >= state_dict["lower_band_radius"]:
                self.current_state = StateMachinee.GO_TO_UPPER_BAND
        ...
        elif self.current_state in {StateMachinee.GO_TO_LOWER_BAND, StateMachinee.GO_TO_UPPER_BAND}:
            state_dict["remaining_time"] -= time() - self.prev_time
            self.prev_time = time()

            if self.current_state == StateMachinee.GO_TO_LOWER_BAND:
                maybe_next_state = StateMachinee.STAY_IN_LOWER_BAND
                side = "right"
            else:
                maybe_next_state = StateMachinee.STAY_IN_UPPER_BAND
                side = "left"
        ...
        elif self.current_state == StateMachinee.PAUSE:
            if time() - state_dict["state_start_time"] >= state_dict["state_wait_time"]:
                self.current_state = StateMachinee.GO_TO_MIDDLE_CIRCLE
        ...
        elif self.current_state == StateMachinee.TRIAL_TERMINATION:
            if time() - state_dict["state_start_time"] >= state_dict["state_wait_time"]:
                self.current_state = StateMachinee.GO_TO_START_CIRCLE
                self.set_go_to_start_circle(state_dict)
                
        elif self.current_state == StateMachinee.EXIT:
            if state_dict["enter_pressed"]:
                continue_loop = False
        ...
        if state_dict["remaining_time"] == 0 and \
           self.current_state not in {StateMachinee.PAUSE, StateMachinee.STAY_IN_LOWER_BAND, StateMachinee.STAY_IN_UPPER_BAND, StateMachinee.GO_TO_LOWER_BAND_AFTER_TRIAL, StateMachinee.GO_TO_UPPER_BAND_AFTER_TRIAL}:
            if state_dict["block_idx"] < state_dict["total_blocks"] - 1:
                self.current_state = StateMachinee.PAUSE
                self.set_pause(state_dict)
                state_dict["needs_update"] = True
            else:
                self.current_state = StateMachinee.EXIT
